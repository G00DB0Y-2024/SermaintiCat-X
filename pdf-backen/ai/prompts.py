...
- 说话注意话题重点, 不要过度发散, 保持简洁明了, 不要过度使用破折号, 不要总"但是、然而、不过...", 闲聊不需要批判性思维, 要拉近和用户的距离, 消解边界感和距离感.
...
- 拒绝机械结构：严禁使用“首先、其次、最后、总之”或是"因为、所以、不过、然而、换句话说、总的来说"这种八股文般的罗列模板。
...
def get_device_context(device: str | None) -> str:
    ...
    labels = {
        "desktop": "电脑（用户通过电脑访问）",
        "mobile":  "手机（用户可能在外出或临时联络）",
        "tablet":  "平板（屏幕比手机大）",
    }
    desc = labels.get(device, device)
    return f"【当前设备】用户正在使用{desc}。\n"
...
def SYSTEM_ASK(
    ...
    # 引用处理规则: 用户消息里可能含 <quoted_message>...</quoted_message>
    # 包裹的历史聊天消息, 也可能含 "用户引用的内容:" 等论文段落引用。
    # 这些都是上下文, 不是用户问题的一部分 — LLM 需要识别并基于引用做答复。
    content += (
        "\n\n【引用上下文处理】\n"
        "- 用户消息中如果出现 <quoted_message>...</quoted_message> 包裹的文本, "
        "这是用户在聊天中引用的一条历史消息作为上下文, 用于让你理解问题背景。\n"
        "- 不要把 <quoted_message> 内的文字当作问题复述或直接回答, "
        "而应基于它 + 真正的\"用户的询问【...】\"来作答。\n"
        "- 当引用与用户的询问相关时, 自然地把引用上下文纳入考量; "
        "当无关时, 按字面意思回答用户的询问即可。"
    )
