...
function maybeRenderPdfDoc(isNotPOST) {
  ...
  const translatorMini = document.createElement('iframe');
  translatorMini.style.width = '400px';
  translatorMini.style.height = '250px'; 
  translatorMini.style.border = 'none';
  translatorMini.style.position = 'absolute';
  translatorMini.style.top = '-300px';
  translatorMini.style.right = '100px';
  translatorMini.style.zIndex = '9999';
  translatorMini.style.pointerEvents = 'none'
  translatorMini.style.boxShadow = 'none'
  translatorMini.style.borderRadius = '10px'
  translatorMini.style.transition = 'all 0.15s ease-in-out';
  translatorMini.src = chrome.runtime.getURL('translator-mini/index.html' + "?ts=" + vid);
  container.appendChild(translatorMini);

  window.addEventListener('message', function(event) {
      if (event.source === translatorMini.contentWindow) {
          if (event.data.type === 'SET_TRANSPARENCY') {
              if( event.data.pointNone){
                translatorMini.style.pointerEvents = 'none'
                translatorMini.style.top = '-300px';
                translatorMini.style.boxShadow = 'none'
              }
              else{
                translatorMini.style.pointerEvents = 'auto'
                translatorMini.style.top = '0px';
                translatorMini.style.boxShadow = '2px 2px 5px rgba(0,0,0,0.5)'
              }
          }
      }
  });
